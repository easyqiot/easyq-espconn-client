...
{
    EasyQSession* eq = (EeasQSession*)e->par;
    ...
    case EASYQ_RECONNECT_REQ:
        break;
    case TCP_RECONNECT:
        easyq_tcpclient_delete(eq);
        easyq_connect(eq);
        INFO("TCP: Reconnect to: %s:%d\r\n", eq->hostname, eq->port);
        eq->tcpconn = EASYQ_CONNECTING;
        break;
    case EASYQ_DELETING:
    ...
    case EASYQ_DISCONNECTED:
        ...
        eqsyq_tcpclient_delete(client);
        break;
    case MQTT_DELETED:
        INFO("MQTT: Deleted client\r\n");
        easyq_client_delete(client);
        break;
    case MQTT_KEEPALIVE_SEND:
        easyq_send_keepalive(client);
        break;
    case MQTT_DATA:
        ...
        if (QUEUE_Gets(&client->msgQueue, dataBuffer, &dataLen, MQTT_BUF_SIZE) == 0) {
            client->easyq_state.pending_msg_type = easyq_get_type(dataBuffer);
            client->easyq_state.pending_msg_id = easyq_get_id(dataBuffer, dataLen);


            client->sendTimeout = MQTT_SEND_TIMOUT;
            INFO("MQTT: Sending, type: %d, id: %04X\r\n", client->easyq_state.pending_msg_type, client->easyq_state.pending_msg_id);
            espconn_send(client->pCon, dataBuffer, dataLen);

            client->easyq_state.outbound_message = NULL;
            break;
        }
        break;
    }
...
easyq_dns_found(const char *name, ip_addr_t *ipaddr, void *arg)
{
    ...
    if (eq->ip.addr == 0 && ipaddr->addr != 0)
    {
        ...
        eq->status = TCP_CONNECTING;
        INFO("TCP: connecting...\r\n");
    }

    system_os_post(MQTT_TASK_PRIO, 0, (os_param_t)eq);
}
...
void ICACHE_FLASH_ATTR easyq_connect(EasyQSession *eq, EasyQCallback callback) {
    ...
    eq->tcpconn->proto.tcp->remote_port = port;
    eq->tcpconn->reverse = eq; 
    espconn_regist_connectcb(eq->conn, easyq_tcpclient_connect_cb);
    espconn_regist_reconcb(eq->conn, easyq_tcpclient_recon_cb);
    ...
    if (UTILS_StrToIP(eq->hostname, &eq->tcpconn->proto.tcp->remote_ip)) {
        ...
        espconn_gethostbyname(eq->tcpconn, eq->host, &eq->ip, easyq_dns_found);
    }
}
...
void ICACHE_FLASH_ATTR easyq_init(EasyQSession * eq, const char *hostname, 
    ...
    QUEUE_Init(&eq->msgQueue, QUEUE_BUFFER_SIZE);
